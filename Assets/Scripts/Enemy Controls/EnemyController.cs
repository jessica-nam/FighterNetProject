...
namespace UnityStandardAssets.Characters.ThirdPerson
{
	...
	public class EnemyController : MonoBehaviour
	{
		...
		private DoubleTapDetector uTapDetector = new DoubleTapDetector();
		private DoubleTapDetector iTapDetector = new DoubleTapDetector();
		private DoubleTapDetector oTapDetector = new DoubleTapDetector();
		private DoubleTapDetector hTapDetector = new DoubleTapDetector();
		private DoubleTapDetector pTapDetector = new DoubleTapDetector();
		...
		public void Update()
		{
			if(uTapDetector.CheckForDoubleTap(KeyCode.U))
            {
				if (m_IsGrounded)
				{
            ...
            else if (uTapDetector.isSingleTapPending && uTapDetector.tapTimer <= 0)
            {
                if (m_IsGrounded)
				{
				...
				attackName = "UpLight";
                uTapDetector.isSingleTapPending = false;
				StartCoroutine(CheckCollisionDurationLightDamage(5.0f));
			}


			if(iTapDetector.CheckForDoubleTap(KeyCode.I))
            {
                if (m_IsGrounded)
				{
			...
			else if(iTapDetector.isSingleTapPending && iTapDetector.tapTimer <= 0)
            {
                if (m_IsGrounded)
				{
				...
				iTapDetector.isSingleTapPending = false;
				StartCoroutine(CheckCollisionDurationLightDamage(5.0f));
            }

			if(oTapDetector.CheckForDoubleTap(KeyCode.O))
            {
                if (m_IsGrounded)
				{
            ...
            else if (oTapDetector.isSingleTapPending && oTapDetector.tapTimer <= 0)
            {
                if (m_IsGrounded)
				{
                ...
                oTapDetector.isSingleTapPending = false;
				StartCoroutine(CheckCollisionDurationLightDamage(6.0f));
			}

			if(hTapDetector.CheckForDoubleTap(KeyCode.H))
            {
                return;
            }
            else if (hTapDetector.isSingleTapPending && hTapDetector.tapTimer <= 0)
            {
                if (m_IsGrounded)
				{
				...
				attackName = "Ariel";
                hTapDetector.isSingleTapPending = false;
				StartCoroutine(CheckCollisionDurationStrongDamage(5.0f));
			}
		...
		IEnumerator CheckCollisionDurationStrongDamage(float duration)
		{
			...
			while (Time.time - startTime < duration)
			{
				...
				{
					dealingDamage = true;
					enemyAtm.DealDamage(playerAtm.gameObject, 10);
		...
		IEnumerator CheckCollisionDurationLightDamage(float duration)
		{
			...
			while (Time.time - startTime < duration)
			{
				...
				{
					dealingDamage = true;
					enemyAtm.DealDamage(playerAtm.gameObject, 5);
					yield break;  // Exit the loop and coroutine since we've dealt damage.
				}
				yield return null;  // Wait for the next frame.
			}
